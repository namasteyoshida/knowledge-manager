...
## 2026-09-13(Day8):Tailwind学習・模擬口頭ディフェンスによる課題洗い出し

### Tailwind CSSの知識固め

**学習内容**

- ユーティリティクラスの命名規則(プロパティ略称-値)と、余白のスケール(数値×4px)の体系を、実際にプロジェクトで使ったクラス(`px-3 py-2`, `gap-4`等)を教材に整理した
- `flex`(1列の整列)と`grid`(行×列のグリッド)の使い分けの判断基準を、記事一覧のヘッダー(flex)と記事作成画面の2カラムレイアウト(grid)を例に確認した
- 状態バリアント(`hover:`, `disabled:`, `focus:`)の仕組みと、プレフィックスの組み合わせ(`disabled:hover:`等)が可能であることを確認した
- Tailwind v4の設定方式(`tailwind.config.js`からCSSファイル内の`@import`/`@theme`/`@plugin`ディレクティブへの移行)を、実際に使っている`@plugin "@tailwindcss/typography"`を例に整理した
- `@theme inline`とCSS変数(`var(--background)`等)を経由したダークモード対応の仕組みを、`create-next-app`のデフォルトテンプレートを教材に理解した
- レスポンシブ対応のブレークポイント(`sm:`〜`2xl:`)とモバイルファーストの考え方を整理した。本プロジェクトでは`grid-cols-2`が375px幅でも崩れなかったため未使用だが、仕組みとしては理解した

### 模擬口頭ディフェンス(第三者AIによる辛口レビュー)の実施

**経緯**

- developブランチ時点のソースコード・要件定義書・設計書・READMEを対象に、小松のAIに口頭ディフェンス想定質問集を作成してもらい、1項目ずつ検証した
- レビューの質は高く、表面的な指摘ではなく「ドキュメントに書いた主張と実装が本当に一致しているか」を突き合わせる内容だった

**洗い出された問題点(A〜H、対応は翌日に実施)**

- A. 検索機能:要件定義書は「最新Revisionのみ」検索と明記しているが、実装は「全Revision(過去の改訂含む)」を検索対象にしており齟齬があった
- B. 「全文検索」という呼称が、実装(単純な部分一致検索、`ILIKE`相当)に対して技術的に誇大であった。性能見積もり(データ件数増加時の3秒以内目標との整合)も未実施だった
- C. `createPage`の「トランザクション」という表現の正確性の検証。`updatePage`は2回の独立した書き込みで、明示的なトランザクション保護がなく、片方が失敗した場合のデータ不整合リスクが未検討だった
- D. `deletePage`だけ、以前の設計方針変更(throwからreturn方式への統一)が適用されておらず、本番ビルドでReact error #441が再発しうる状態だった
- E. Gamma連携の非同期処理に3つの穴:(1)`processGeneration`の最初のDB更新がtry/catchの外にあり失敗時にリクエストがPENDINGのまま止まる、(2)`GammaGenerator`のポーリングが無限ループでタイムアウトがない、(3)モック実装のみの検証で実際の失敗パターン(レート制限、不正なレスポンス等)が未検証
- F. なりすまし対策ゼロという判断を「学習目的だから」で正当化することの論理的整合性、および属人化解消という目的と実装の矛盾についての指摘
- G. パフォーマンス・データ設計:(1)記事詳細ページで不要な全Revisionのcontentを取得している、(2)一覧・検索にページネーションがない
- H. 見送り基準(diff/ロールバック)とストレッチ機能実装基準(Gamma、モックで完了扱い)の整合性

**各指摘への一次評価**

- A・C(updatePage側)・D・E-1・E-2・G(全項目)は、実際に見落としていた不備として認め、修正することとした
- B・C(createPageの表現)は、コード修正ではなくドキュメントの用語・記述の精緻化で対応することとした
- Fは詭弁性が高いと判断し、指摘の前提(なりすましの動機がほぼ存在しない、社内利用という脅威モデル)を検証した上でコード・ドキュメントとも変更しない方針とした。
- Hは、H-1(見送り基準とストレッチ実装基準の違い)は「仕様不明で未着手」と「仕様は明確だが外部要因で未検証」という質的な違いがあるとして反論可能と判断。H-2(優先順位の判断)は正当な指摘として認めた

---

## 2026-09-14(Day9):模擬口頭ディフェンスで判明した課題の修正

### A. 検索仕様の是正

**対応内容**

- 要件定義書の記述(最新Revisionのみ検索)に実装を合わせる方針とした
- Prismaの標準的な`where`句だけでは「各Pageの最新Revisionに限定した検索」を直接表現できないため、削除済みを除く全記事を最新Revision付きで取得した上で、アプリケーション側(JavaScript)でタイトル・最新本文への部分一致を判定する方式(`fetchMatchedPages`関数)に変更した
- この方式は記事数増加時に性能が劣化する限界を持つ(全件取得が発生するため)。小規模利用前提のプロトタイプとして許容し、限界は設計書に明記する方針とした
- ロールバック機能を将来実装する際は、「過去の改訂を検索対象に含める」という以前の設計(`revisions: { some: {...} } `)をその文脈で再検討する余地があることをメモとして残した

### C. `updatePage`のトランザクション保護

**対応内容**

- `prisma.revision.create`と`prisma.page.update`の2回の独立した書き込みを、`prisma.$transaction([...])`で明示的に囲む修正を行った
- `createPage`(Prismaのネストしたcreateによる暗黙的トランザクション)との非対称性を解消し、両者が同じ整合性レベルを持つようにした
- あわせて、`createPage`の「トランザクション」という表現についても、「`$transaction`を明示的に使っているのではなく、Prismaのネスト書き込みが内部でトランザクションとして扱われることに依拠している」という技術的な正確さを補う説明が必要であることを確認した

### D. `deletePage`のエラー通知方式の統一

**対応内容**

- `createPage`・`updatePage`・`createPageFromTemplate`にのみ適用されていた「throwではなくreturn { error }で通知する」という設計方針を、見落としていた`deletePage`にも適用した
- 対象記事が存在しない場合は`{ error: "対象の記事が見つかりません。" }`を返すよう修正し、`DeleteButton.tsx`側もtry/catch(throw前提)から戻り値チェック方式に修正した
- これにより、以前解決したはずのReact error #441相当の問題が、`deletePage`という別の場所で再発しうる状態だったことが判明し修正した
- 学習:設計方針を変更する際は、対象となる全箇所(全Server Actions)を横断的にリストアップして適用漏れがないか確認する必要がある

### E-1. `processGeneration`のエラーハンドリング強化

**対応内容**

- `processGeneration`内の最初のDB更新(PENDING→PROCESSING)が`try`ブロックの外にあり、この処理自体が失敗した場合にunhandled promise rejectionとなり、リクエストがPENDINGのまま無限に残ってしまう不具合を修正した
- 関数全体を1つの`try/catch`で囲み、失敗時は`FAILED`として記録するようにした。catchブロック内のDB更新自体がさらに失敗する可能性は残るため、その場合は`console.error`でログに残す設計とした

### E-2. `GammaGenerator`のポーリングにタイムアウトを追加

**対応内容**

- `while (true)`による無限ポーリングを、最大試行回数(24回、5秒間隔で最大2分)によるタイムアウト付きループに修正した
- Vercelのサーバーレス実行時間制約(設計書に既記載)とは別に、ローカル環境でもGamma側が永久に`processing`を返し続けた場合にループが終わらないという、環境を問わないコード自体の欠陥だったことを確認した
- タイムアウトの具体的な秒数(2分)は、Gamma側の実際の生成時間を検証した上での数値ではなく暫定値であることを明記する

### E-3. 未検証であるが故の未知の失敗モードについて

- モック実装のみの検証であるため、Gamma APIの不正なレスポンス、レート制限、ネットワークタイムアウト等、他にも未検証の失敗モードが存在する可能性を否定できないことを確認した。コード修正はE-1・E-2に留め、この限界についてはドキュメントに正直に明記する方針とした

### F. なりすまし対策についての反論の整理

**対応内容(コード変更なし)**

- 指摘(なりすまし対策ゼロが属人化解消という目的と矛盾する)について検証した結果、「技術的に可能であること」と「現実的な脅威として対策すべきこと」を混同した論法であると判断した
- 社内利用という利用者の範囲、なりすましを行う動機がほぼ存在しないこと、対策コスト(認証実装)との費用対効果を踏まえ、対策しないという判断は妥当と結論づけた

### G-1. 記事詳細ページのクエリ最適化

**対応内容**

- 記事詳細ページで、画面上使用しない全Revisionの`content`(本文全体)を`take`なしで取得していた無駄を修正した
- `page`・`latestRevision`(本文含む)・`revisionHistory`(本文を含まない、著者名と日時のみ)の3クエリに分割し、`Promise.all`で並列実行する構成に変更した
- 以前確立した「N+1問題を避けるため1クエリにまとめる」という設計哲学とは逆方向の判断(あえて複数クエリに分割)だが、今回は「不要なデータを読まないこと」を優先した

### H. スコープ判断の一貫性についての整理

**対応内容(コード変更なし)**

- H-1(見送り基準とストレッチ実装基準の整合性)については、「仕様設計自体が固まっていないため未着手(diff/ロールバック)」と「仕様は明確で実装済みだが外部の契約制約により実環境検証のみできない(Gamma連携)」という、質的に異なる状態を同列に比較すること自体が不公平な比較であると整理し、反論可能と判断した
- H-2(優先順位の判断)については、Aで発覚した検索機能の齟齬が、Gamma連携着手前から実際にコードとして存在していたにもかかわらず気づかず「検索機能は完了」と判断していたことを踏まえ、MVPの受け入れテストの精度が不十分だったという指摘を正当なものとして認めた

### 総括

- 今回の模擬口頭ディフェンスを通じて、「動作すること」と「要件定義書に書いた通りに動作すること」は別軸であるという教訓を得た。特にA(検索仕様の齟齬)・D(deletePageの適用漏れ)・G-1(不要なデータ取得)は、いずれも「一見動いているように見えるが、詳細に検証すると綻びがある」という共通点を持っており、機能実装後の受け入れテストは正常系だけでなく、ドキュメントとの整合性・異常系・パフォーマンス特性まで含めて行う必要があることを確認した

## Mainブランチにpush後

- テスト・テストドキュメント制作やコードの詳細理解を予定している